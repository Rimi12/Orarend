...
export interface AssistantSlot {
  ...
  timeSlotIndex: number; // index into timeSlots array
  locationIndex: number; // index into locations array
  customStartTime?: string; // e.g. "07:15"
  customEndTime?: string;   // e.g. "08:15"
  customDurationMinutes?: number; // optional override
  note?: string;
}
...
export interface AssistantSchedule {
  ...
  timeSlots?: string[]; // editable time slots array
  assistantNames?: Record<string, string>; // optional custom name mappings: assistantId -> name
}