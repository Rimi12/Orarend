...
export const useDataManagement = (googleDrive: any) => {
  ...
  const handleLoadStateFromJson = useCallback((jsonString: string) => {
    try {
        ...
        if (savedState.assistantSchedule) {
          localStorage.setItem('assistantScheduleState', JSON.stringify(savedState.assistantSchedule));
          window.dispatchEvent(new Event('assistantScheduleUpdated'));
        }
    } catch (error) {
        alert("Hiba a mentési fájl feldolgozása közben. A fájl sérült vagy nem megfelelő formátumú.");
    }
  }, [loadState]);
  ...
  const handleSaveToDrive = async () => {
    ...
    try {
      let assistantSchedule: any = undefined;
      try {
        const rawAssistant = localStorage.getItem('assistantScheduleState');
        if (rawAssistant) assistantSchedule = JSON.parse(rawAssistant);
      } catch {}

      const stateToSave: SavedState = { ...currentState, version: '3.1.0', assistantSchedule };
